...
- **Every number we made up is now tagged and enumerated** (`design/params.py`, and
  ...
  | `FINGERTIP_BREADTH` | 25/20/20/20/15 mm | fingertip width across the nail — thumb, index, middle, ring, little. **Measured, but n=1**: calipers on one adult male hand. It replaces MyoHand's own distal-phalanx capsules, which are a slim model and printed cups **12.0–15.4 mm** across — below any adult finger, and the first gauntlet could not be worn. ⚠ **This is now a population claim resting on one hand.** It is not a hand-LENGTH error (matching breadth by scaling length implies a 278 mm hand), so the 5th/50th/95th percentile model still scales these by hand length and has no independent evidence that finger breadth scales that way. Real anthropometric finger-breadth data would replace it. It is load-bearing: with 20 mm cups the five wells only pack in a narrow band of postures, and `key-overlap`/`well-finger` become binding constraints they were not before. |
