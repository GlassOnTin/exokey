...
- **Every number we made up is now tagged and enumerated** (`design/params.py`, and
  ...
  | `E_HOT` | 20 MPa | effective stiffness of the just-laid FDM bead while still above T_g and able to sag — ~3 orders below the 6 GPa solid. Load-bearing guess of the print-sag model in `support_mm(hot=)`: the self-supporting hot-bridge span goes as `E_HOT^(1/4)`, so at 2–20 MPa it brackets 8–15 mm around the old fixed 10 mm rule. ⚠ **Used only for a POST-HOC support estimate, not an optimiser objective** — an in-loop print-support objective built on this surrogate was tried and DROPPED because it did not predict real PrusaSlicer support (n=8, pearson −0.06; real support is driven by mesh surface + orientation, not the skeletal line model). The hard printability constraint stays on the conservative 45°/10 mm heuristic. |
  | `SAG_TOL` | 0.1 mm | how far a hot bridge may droop before the print degrades — taken as half a layer. Paired with `E_HOT` in that same post-hoc support estimate; nothing has printed one to find the real limit. |
  | `DON_CLEAR` | 2 mm | the room a finger needs BESIDE it along its donning corridor, over and above not intersecting the part. ⚠ The entry constraint used to demand only `gap >= -TOUCH_TOL` — that the gauntlet not push more than 0.3 mm *into* the finger. That is a no-interference test, not a fit test, and a mass-minimising optimiser parks exactly on whatever line it is given: nineteen independent designs all came back clearing by 0.00–0.10 mm, which the model called feasible and a hand called jammed. 2 mm per side is an estimate covering FDM tolerance (~0.2 mm), a hand that cannot be perfectly aligned on the way in, and a knuckle that is bone rather than compressible pulp — **not a measurement**. The honest way to set it is a printed corridor coupon: print the entry region alone at several clearances and find where a hand stops binding. |
  | `DON_LEN` | 80 mm | how far back down the donning path the corridor is checked. ⚠ It was a fixed 20 mm, so the model inspected only the last stretch of insertion — clear by construction on a seated design — and never looked at the 20–70 mm the hand actually travels through. Measured on the first printed gauntlet, that unchecked region was obstructed for EVERY finger (−0.03 to −1.10 mm) while the checked 20 mm read +0.4 to +1.7 mm: the model called the device donnable and the hand could not get into it. 80 mm is where the clearance profile went clear on that design, not a measurement of a real approach; a longer real path would need more. Paired with the trajectory model — the hand enters EXTENDED and curls as the tips seat — which is itself a simplification: fingers are checked on one shared rigid approach, so a real hand fouling one finger and twisting to compensate is not represented. |
  | `CRADLE_CLEAR` | 1.5 mm | the room the drop-in TPU cradle needs around it, clear of the grown structure. ⚠ The cradle is the MOVING part — the fingertip tilts it over the Hall sensor — and **nothing reserved its envelope**: the structure was checked against the finger and allowed to fuse into the rigid frame (which is intended, the button strut must tie in), but never checked against the part that has to travel, and the export carves only the PCB slot and magnet pocket. Measured on a shipped design before this existed: three of five cradles compromised — little BLOCKED by a strut 2.10 mm inside it, thumb and ring clear by only 0.26/0.27 mm, which FDM tolerance (~0.2 mm) closes. Those three keys would not have sensed at all. 1.5 mm is SWITCH_TRAVEL's plunge taken as a plausible swing envelope, **not a measurement** of the cradle's real motion (`CRADLE_LEVER`, itself a guess, puts the lateral component near 1 mm). Re-derive from a printed cradle's measured travel. |
  ...
  | `CARRIER_STANDOFF` | 12 mm | height of the carrier's mount plane above the dorsal skin — the deck the kit bolts to. **Our bracket's choice, not a kit dimension** (the kit's own stack depth is `KIT_ENV_T`, still UNMEASURED). It sets the payload's lever arm off the wrist anchor and must fit the ~10 mm wearable stack budget once the grown shell is counted. Replaced by a DERIVED value off the measured `KIT_ENV_T` when the kit arrives. |
  | `CARRIER_TOWER` | 10 mm | how far each keywell sits above the mount plane — the tower the key rides on so the fingertip reaches it from the typing posture. **Our bracket's choice**; the kit's own tower bracket supersedes it on arrival. |
  | `CARRIER_MASS` | 80 g | the mass budget the carrier is sized to carry for the static load case. **Our bracket's estimate** (a Svalboard cluster is hand-held light); the real number is `KIT_MASS` off the kitchen scale, and shipping uses that. |
  | `CARRIER_COM_D` / `CARRIER_COM_R` | 0.55 / 0.50 | the payload centre of mass as a fraction along (distal) and across (index-ward) the deck footprint. **Our bracket's guess** at where the keywell mass sits; the weighed assembly replaces it. It decides how the payload moment loads the wrist anchor. |
